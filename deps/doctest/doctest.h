...
#define DOCTEST_SUPPRESS_COMMON_WARNINGS_PUSH                                                      \
        ...
        DOCTEST_MSVC_SUPPRESS_WARNING( 4512 ) /* 'class' : assignment operator could not be        \
                                                 generated */                                      \
        ...
        DOCTEST_MSVC_SUPPRESS_WARNING( 4640 ) /* construction of local static object not           \
                                                 thread-safe */                                    \
        ...
        DOCTEST_MSVC_SUPPRESS_WARNING( 26444 ) /* Avoid unnamed objects with custom ctor and       \
                                                  dtor... */                                       \
...
#define DOCTEST_MAKE_STD_HEADERS_CLEAN_FROM_WARNINGS_ON_WALL_BEGIN                                 \
        ...
        DOCTEST_MSVC_SUPPRESS_WARNING( 5039 ) /* pointer to pot. throwing function passed to       \
                                                 extern C */                                       \
        DOCTEST_MSVC_SUPPRESS_WARNING( 5045 ) /* Spectre mitigation for memory load */             \
        DOCTEST_MSVC_SUPPRESS_WARNING( 5105 ) /* macro producing 'defined' has undefined behavior  \
                                               */                                                  \
        DOCTEST_MSVC_SUPPRESS_WARNING( 4738 ) /* storing float result in memory, loss of           \
                                                 performance */                                    \
...
#define DOCTEST_CMP_LE( l, r ) le( l, r )
        ...
        struct Expression_lhs
        {
                ...
                DOCTEST_NOINLINE operator Result()
                {
                        ...
                        DOCTEST_MSVC_SUPPRESS_WARNING_WITH_PUSH( 4800 )  // 'int': forcing value to
                                                                         // bool
        ...
        struct DOCTEST_INTERFACE ExpressionDecomposer
        {
                ...
                // The right operator for capturing expressions is "<=" instead of "<<" (based on
                // the operator precedence table) but then there will be warnings from GCC about
...
#define DOCTEST_ASSERT_IN_TESTS( decomp )                  \
            ...
            const DOCTEST_REF_WRAP( R ) rhs )
        {
                ...
                // ###################################################################################
                // IF THE DEBUGGER BREAKS HERE - GO 1 LEVEL UP IN THE CALLSTACK FOR THE FAILING
                // ASSERT THIS IS THE EFFECT OF HAVING 'DOCTEST_CONFIG_SUPER_FAST_ASSERTS' DEFINED
                // ###################################################################################
...
struct DOCTEST_INTERFACE IReporter
{
        ...
        // called when the whole test run ends (caching a pointer to the input doesn't make sense
        // here)
        ...
        // decorator or isn't in the execution range (between first and last) (safe to cache a
        // pointer to the input)
...
DOCTEST_MSVC_SUPPRESS_WARNING( 4530 )  // C++ exception handler used, but unwind semantics not
                                       // enabled
DOCTEST_MSVC_SUPPRESS_WARNING( 4577 )  // 'noexcept' used with no exception handling mode specified
DOCTEST_MSVC_SUPPRESS_WARNING( 4774 )  // format string expected in argument is not a string literal
DOCTEST_MSVC_SUPPRESS_WARNING( 4365 )  // conversion from 'int' to 'unsigned', signed/unsigned
                                       // mismatch
DOCTEST_MSVC_SUPPRESS_WARNING( 5039 )  // pointer to potentially throwing function passed to extern
                                       // C
DOCTEST_MSVC_SUPPRESS_WARNING( 4800 )  // forcing value to bool 'true' or 'false' (performance
                                       // warning)
DOCTEST_MSVC_SUPPRESS_WARNING( 5245 )  // unreferenced function with internal linkage has been
                                       // removed
       ...
       // is slower because all atomics have to be accessed.
        ...
        class MultiLaneAtomic
        {
                struct CacheLineAlignedAtomic
                {
                        Atomic< T > atomic{};
                        char        padding
                            [DOCTEST_MULTI_LANE_ATOMICS_CACHE_LINE_SIZE - sizeof( Atomic< T > )];
                };
        ...
        private:
                ...
                // threads use this, some will use the same atomic. So performance will degrade a
                // bit, but still everything will work.
                //
        ...
        struct ContextState : ContextOptions, TestRunStats, CurrentTestCaseStats
        {
                ...
                std::vector< String > stringifiedContexts;  // logging from INFO() due to an
                                                            // exception
...
String& String::operator+=( const String& other )
{
        ...
        } else if ( data.capacity > total_size ) {
...
{
        if ( isOnStack() )
                return last - ( size_type( buf[last] ) & 31 );  // using "last" would work only if
                                                                // "len" is 32
        return data.size;
}
DOCTEST_GCC_SUPPRESS_WARNING_POP
...
namespace detail
{
          ...
          : m_signature( { name, file, line } )
        {
                ...
                } else if ( g_cs->subcaseStack[g_cs->currentSubcaseDepth] == m_signature ) {
        ...
        DOCTEST_MSVC_SUPPRESS_WARNING_WITH_PUSH( 4996 )  // std::uncaught_exception is deprecated in
                                                         // C++17
...
namespace detail
{
        ...
        {
                if ( std::find(
                         getExceptionTranslators().begin(), getExceptionTranslators().end(), et ) ==
                     getExceptionTranslators().end() )
                        getExceptionTranslators().push_back( et );
        }

        DOCTEST_THREAD_LOCAL std::vector< IContextScope* > g_infoContexts;  // for logging with
                                                                            // INFO()
        ...
        struct FatalConditionHandler
        {
                ...
                FatalConditionHandler()
                {
                        ...
                        std::set_terminate( []() DOCTEST_NOEXCEPT {
                                ...
                                if ( isDebuggerActive() && !g_cs->no_breaks )
                                        DOCTEST_BREAK_INTO_DEBUGGER();
                                std::exit( EXIT_FAILURE );  // explicitly exit - otherwise the
                                                            // SIGABRT handler may be called as well
                        } );
...
namespace detail
{
        ...
        {
                if ( m_severity & assertType::is_require )  //! OCLINT bitwise operator in
                                                            //! conditional
                        throwException();
        }
}  // namespace detail
...
// =================================================================================================
        ...
        void fulltext_log_assert_to_stream( std::ostream& s, const AssertData& rb )
        {
                ...
                } else if ( rb.m_at & assertType::is_throws_as ) {  //! OCLINT bitwise operator in
                                                                    //! conditional
                        s << Color::Cyan << assertString( rb.m_at ) << "( " << rb.m_expr << ", "
        ...
        struct ConsoleReporter : public IReporter
        {
                ...
                virtual void
                file_line_to_stream( const char* file, int line, const char* tail = "" )
                {
                        s << Color::LightGrey << skipPathFromFilename( file )
                          << ( opt.gnu_file_line ? ":" : "(" )
                          << ( opt.no_line_numbers ? 0 : line )  // 0 or the real num depending on
                                                                 // the option
...
#define DOCTEST_DEBUG_OUTPUT_REPORTER_OVERRIDE( func, type, arg )         \
        ...
        bool
        parseOptionImpl( int argc, const char* const* argv, const char* pattern, String* value )
        {
...
#define DOCTEST_PARSE_INT_OPTION( name, sname, var, default )                                \
        if ( parseIntOption(                                                                 \
                 argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX name "=", option_int, intRes ) || \
             parseIntOption(                                                                 \
...
#define DOCTEST_PARSE_STR_OPTION( name, sname, var, default )                               \
        if ( parseOption(                                                                   \
                 argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX name "=", &strRes, default ) ||  \
             parseOption(                                                                   \
                 argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX sname "=", &strRes, default ) || \
             withDefaults )                                                                 \
        p->var = strRes
...
int Context::run()
{
        ...
        // (because reset() is a static method)
                                ...
                                // exit this loop if enough assertions have failed - even if there
                                // are more subcases
                                if ( p->abort_after > 0 &&
...
DOCTEST_MSVC_SUPPRESS_WARNING_WITH_PUSH( 4007 )  // 'function' : must be 'attribute' - see issue
                                                 // #182
